...
const Profile = () => {
  ...
  }, [user]) // whenever user changes its executes
    ...
    <div className="page-container">
      ...
      <div className="profile-content">
        ...
        <Card>
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              ...
              <p className="text-slate-800 dark:text-slate-400 text-xs">Member Since {new Date(user?.createdAt || '').toLocaleDateString()} </p>
            </div>
          </div>
          ...
          {isEditing ? (
            <div className="space-y-4">
              ...
              <Select
                label="Fitness Goal"
                value={formData.goal}
                onChange={(value) => setFormData({ ...formData, goal: value as 'lose' | 'maintain' | 'gain' })} // 'lose' | 'maintain' | 'gain' these are type assertion that the values will be from one of these
                options={goalOptions}
              />
                ...
                <Button
                  onClick={() => {
                    ...
                    setFormData({
                      ...
                      dailyCalorieIntake: user.dailyCalorieIntake || 2000,
                      dailyCalorieBurn: user.dailyCalorieBurn || 400
                    }) // sets the form data with user entered data
                  }}
                  variant="secondary"
                  className="flex-1">
                  Cancel
            ...
            <>
              <div className="space-y-4">
                ...
                <div className="flex items-center gap-4 p-3 bg-slate-50 dark:bg-slate-800 rounded-lg transition-colors duration-200">
                  ...
                  <div className="">
                    <p className="text-s. text-slate-500 dark:text-slate-400">Age</p>
                    <p className="font-bold text-slate-800 dark:text-white"> {user.age} year's</p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-4 p-3 bg-slate-50 dark:bg-slate-800 rounded-lg transition-colors duration-200">
                  ...
                  <div className="">
                    <p className="text-s. text-slate-500 dark:text-slate-400">Weight</p>
                    <p className="font-bold text-slate-800 dark:text-white"> {user.weight} kg</p>
                  </div>
                </div>
                ...
                {user.height !== 0 && (
                  <div className="flex items-center gap-4 p-3 bg-slate-50 dark:bg-slate-800 rounded-lg transition-colors duration-200">
                    ...
                    <div className="">
                      <p className="text-s. text-slate-500 dark:text-slate-400">Height</p>
                      <p className="font-bold text-slate-800 dark:text-white"> {user.height} cm</p>
                    </div>
                  </div>
                )}
                ...
                <div className="flex items-center gap-4 p-3 bg-slate-50 dark:bg-slate-800 rounded-lg transition-colors duration-200">
                  ...
                  <div className="">
                    <p className="text-s. text-slate-500 dark:text-slate-400">Goal</p>
                    <p className="font-bold text-slate-800 dark:text-white"> {goalLabels[user.goal || 'gain']}</p>
                  </div>
                </div>
              </div>
              <Button
                ...
                variant="secondary"
                className="w-full mt-4">Edit Profile</Button>
            </>
          )}
        ...
        <div className="space-y-4">
          ...
          {/* Toggle theme button for mobile devices */}
