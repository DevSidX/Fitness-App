...
import { FlameIcon, HamburgerIcon, ActivityIcon, ZapIcon, TrendingUpIcon, ScaleIcon, Ruler } from "lucide-react"
...
const Dashboard = () => {
  ...
  // load user data
  const loadUserData = () => {
    const todayDate = new Date().toISOString().split('T')[0]  // date

    const foodData = allFoodLogs.filter((food: FoodEntry) => food.createdAt?.split('T')[0] === todayDate) // creates a new array containing only the food log entries that were created on a specific date
    setTodayFood(foodData)

    const activityData = allActivityLogs.filter((activity: ActivityEntry) => activity.createdAt?.split('T')[0] === todayDate) // creates a new array containing only the food log entries that were created on a specific date
  ...
  const totalCalories: number = todayFood.reduce((sum, item) => sum + item.calories, 0) // sum is running total and item is the current food object
  ...
  const totalBurned: number = todayActivities.reduce((sum, item) => sum + (item.calories || 0), 0) // total calories burned
    ...
    <div className="page-container">
      ...
      <div className="dashboard-grid">
        ...
        <Card className="shadow-lg col-span-2">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="text-right">
              <p className="text-sm text-slate-500 dark:text-slate-500">Limit</p>
          ...
          <ProgressBar value={totalBurned} max={user?.dailyCalorieIntake || 400} />
        </Card>
        ...
        <div className="dashboard-card-grid">
          ...
          {/* Goal Card */}
          {user && (  // this card will span both row and column
            <Card className="bg-linear-to-r  from-slate-200 to-slate-100  dark:from-slate-800 dark:to-slate-700 mt-3">
              <div className="flex items-center gap-4">

                <div className="w-12 h-12 rounded-xl bg-white/10 flex items-center justify-center">
                  <TrendingUpIcon className="w-6 h-6 text-emerald-400 dark:text-emerald-400" />
                </div>
                <div className="font-semibold text-slate-900 dark:text-white capitalize">
                  <p className="text-slate-500 dark:text-slate-400 text-sm">Your Goal</p>
            ...
            <Card className="mt-3">
              <div className="flex items-center gap-4 mb-6">
                <div className="w-12 h-12 rounded-xl bg-indigo-100 flex items-center justify-center">
                  <ScaleIcon className="w-6 h-6 text-indigo-500 dark:text-indigo-400" />
                </div>
                <div>
                ...
                {user.height && (
                  <div className="p-2 border-t border-slate-100 dark:border-slate-800">
                    <div className="flex justify-between items-center mb-2">
                      ...
                      {(() => {
                        const bmi = (user.weight / Math.pow(user.height / 100, 2)).toFixed(1);  // to fixed is for decimal
          ...
          {/* Summary */}
          <Card className="mt-3">
            <h3 className="font-semibold text-slate-800 dark:text-white mb-4 ">Todays's Summary</h3>
